...
int smblib_vbus_regulator_enable(struct regulator_dev *rdev)
{
	struct smb_charger *chg = rdev_get_drvdata(rdev);
	int rc;
#ifdef CONFIG_MACH_REALME_TRINKET//Fanhong.Kong@ProDrv.CHG,add 2018/06/02 for SVOOC OTG
	struct oppo_chg_chip *chip = g_oppo_chip;
				     ...
				     const union power_supply_propval *val)
{
	int rc = 0;
	u8 power_role;
#ifdef CONFIG_MACH_REALME_TRINKET  /* zhangkun@BSP.CHG.Basic, 2019/02/28, Add for disable CC detect*/
	u8 stat = 0;
	int level = 0;
	struct oppo_chg_chip *chip = g_oppo_chip;
#endif
...
#ifdef CONFIG_MACH_REALME_TRINKET  /* zhangkun@BSP.CHG.Basic, 2019/02/22, Add for disable water detect*/
       if (chg->lpd_disabled)
		return false;
#endif
	rc = smblib_read(chg, TYPE_C_SRC_STATUS_REG, &stat);
...
static void typec_src_removal(struct smb_charger *chg)
{
	...
	#ifndef CONFIG_MACH_REALME_TRINKET  /* zhangkun@BSP.CHG.Basic, 2019/02/22, Add for charge*/
	vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,
			is_flash_active(chg) ? SDP_CURRENT_UA : SDP_100_MA);
	#else	
	vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, false, 0);
...
irqreturn_t typec_or_rid_detection_change_irq_handler(int irq, void *data)
{
	struct smb_irq_data *irq_data = data;
	struct smb_charger *chg = irq_data->parent_data;
	#ifndef CONFIG_MACH_REALME_TRINKET  /* zhangkun@BSP.CHG.Basic, 2019/02/22, Add for disable water detect*/
	u8 stat;
	int rc;
	#endif
...
static int smb5_configure_typec(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(chg, USBIN_LOAD_CFG_REG,
		...
		0);
#ifdef CONFIG_MACH_REALME_TRINKET  /* zhangkun@BSP.CHG.Basic, 2019/03/25, Add for compatible AoKangDa DP cable*/
	if (rc < 0)
	{
		dev_err(chg->dev,
...
static int smb5_init_hw(struct smb5 *chip)
{
	struct smb_charger *chg = &chip->chg;
	int rc, type = 0;
    #ifndef CONFIG_MACH_REALME_TRINKET /* Jianchao.Shi@BSP.CHG.Basic, 2017/04/06, sjc Modify for charging */
	u8 val = 0, mask = 0;
    #else
	u8 val = 0;
    #endif
...
#ifdef CONFIG_MACH_REALME_TRINKET//Fanhong.Kong@ProDrv.CHG,add 2018/04/20 for SVOOC
